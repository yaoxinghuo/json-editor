...
export const messages: Record<Lang, Dict> = {
  'zh-CN': {
    ...
    'modal.close': '关闭',
    'modal.hint': '在下方粘贴 curl 命令（支持 # 注释与 \\ 换行），系统会自动解析 URL、请求方法、请求头（含 Authorization）与请求体。',
  ...
  'en-US': {
    ...
    'modal.close': 'Close',
    'modal.hint': 'Paste a curl command below ("#" comments and "\\" line continuations are supported). URL, method, headers (including Authorization), and body will be parsed automatically.',
    'modal.placeholder': "curl -X GET 'https://api.example.com/data' -H 'Authorization: Bearer token123'",
    ...
    'toast.urlLoaded': 'Loaded from URL',
